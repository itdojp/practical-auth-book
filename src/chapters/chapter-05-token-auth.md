...
## 5.0 実務レビューゲート: JWTとトークン保管

JWTは形式仕様（RFC 7519）だけで安全性が決まるわけではない。実装時は JWT BCP（RFC 8725）と OWASP のトークン保管指針を前提に、次を設計レビューの入口にする。

- 署名アルゴリズムは allowlist で固定し、受信した `alg` を信頼して検証方法を切り替えない。`none` や想定外アルゴリズムを許可しない。
...
## 5.0 Runnable minimum: トークン検証の最小確認

トークン方式を採用する場合は、受け入れるトークンと拒否するトークンを同時に確認する。

- **入力**: ローカル検証用の JWT、公開鍵または JWKS、期待する `iss` / `aud` / `exp` / `nbf` / `sub`。
- **起動条件**: 本番鍵や本番 IdP の client secret を使わず、短寿命の検証用鍵とダミークレームを使う。
- **期待結果**: 正しい署名・issuer・audience・期限のトークンだけを受け入れ、期限切れ、audience 不一致、未知の `kid`、`alg=none` は拒否する。
- **確認方法**: 正常系 1 件と拒否系 3 件以上の結果をテストログまたは PR 証跡に残す。
