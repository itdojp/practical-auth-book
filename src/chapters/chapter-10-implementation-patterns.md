# 第10章 実装パターンとベストプラクティス

## なぜこの章が重要か

認証認可の理論を理解しても、実装段階で多くの落とし穴があります。セキュリティホール、パフォーマンス問題、保守性の低下など、実装の品質がシステム全体の成否を左右します。この章では、実戦で培われたパターンとベストプラクティスを学び、堅牢で保守性の高い認証認可システムを実装する方法を習得します。

## 10.0 Runnable minimum: 実装パターンの最小確認

実装章のコード例を読むときは、1 つのログイン必須エンドポイントと 1 つの権限必須エンドポイントで挙動を確認する。

- **入力**: ローカル検証用ユーザー、権限または scope、ログイン必須 API、権限必須 API。
- **起動条件**: 本番ユーザー、本番 token、本番 client secret を使わず、ローカル専用のダミーデータを使う。
- **期待結果**: 未認証は 401、権限不足は 403、成功時は監査ログに user / client / scope / correlation ID が残る。
- **確認方法**: 正常系、未認証、権限不足、期限切れ credential の 4 ケースを最小テストとして記録する。
                ...
                class WebAuthnFlow:
                    ...
                    async def registration_begin(self, user: User) -> dict:
                        ...
                        options = generate_registration_options(
                            ...
                            # セキュリティ設定
                            attestation="none",  # 一般向け Passkey では認証器固有の証明を要求しない
