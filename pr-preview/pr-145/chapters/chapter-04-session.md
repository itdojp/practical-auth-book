...
HTTPプロトコルのステートレス性は、Webの成功の鍵でしたが、同時に大きな課題も生みました。ユーザーがログインした状態を維持するには、何らかの仕組みが必要です。この章では、セッション管理の本質を理解し、なぜ多くのセキュリティ侵害がセッション管理の不備から生じるのかを学びます。実装を通じて、セキュアなセッション管理の方法を身につけましょう。

## 4.0 Runnable minimum: セッション管理の最小確認

実装に入る前に、ローカルまたは検証用環境で次の最小確認を行う。

- **入力**: ダミーユーザーでログインし、セッション Cookie を発行する検証用エンドポイント。
- **起動条件**: 本番の Cookie 名、署名鍵、ユーザー情報を使わず、ローカル専用の秘密値と HTTPS 相当の検証環境を使う。
- **期待結果**: `HttpOnly`、`Secure`、`SameSite`、有効期限、ローテーション、ログアウト時の失効方針が説明できる。
- **確認方法**: ブラウザ開発者ツールまたは `curl -I` で `Set-Cookie` を確認し、期限切れ・改ざん・ログアウト後のセッションが拒否されることを記録する。
...
class JWTSessionManager:
    ...
    def _schedule_cleanup(self, jti: str, exp):
        ...
        if isinstance(exp, datetime):
            delay = max((exp - datetime.utcnow()).total_seconds(), 0)
        else:
