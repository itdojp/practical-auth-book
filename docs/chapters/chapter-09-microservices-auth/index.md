...
マイクロサービス環境では、認証・認可の実装が分散し、信頼境界や一貫した認可判断、性能要件などの課題が増えます。この章では、分散環境特有の論点と、それらを解決する代表的なパターンを整理します。

## 9.0 Runnable minimum: サービス間認証の最小確認

マイクロサービス構成では、ユーザー認証とサービス間認証を混同しないことを最小確認にする。

- **入力**: ユーザー向け API 1 件、内部 API 1 件、サービス間呼び出し 1 本を含む検証シナリオ。
- **起動条件**: API Gateway、BFF、内部サービスのどこで認証・認可・監査を担うかを分ける。
- **期待結果**: 外部リクエストはユーザー認証、内部呼び出しはサービス認証として扱い、必要に応じて token exchange または service identity を使う。
- **確認方法**: 401 / 403 の切り分け、correlation ID、監査ログ、過剰なユーザートークン転送がないことを確認する。
