...
OAuth 2.0は、外部サービス連携やAPIアクセス権限の委任で広く使われる認可プロトコルです。一方で、その柔軟性ゆえに誤実装が起きやすく、セキュリティインシデントの要因になります。この章では、OAuth 2.0が解決する問題、正しいフローの理解、実装時の注意点を整理します。

> **この章で学ぶこと**
> - OAuth 2.0が生まれた背景（パスワード共有のアンチパターン等）と設計思想  
> - 代表的なグラントタイプ（特に Authorization Code + PKCE と Client Credentials）の使い分け  
> - 実装で押さえるべきポイント（state, PKCE, トークンの有効期限と保存場所など）
            ...
            'problems': [
                {
                    ...
                    'example': 'サードパーティアプリ経由で認証情報が漏えいした事例が報告されることがある（要確認）'
                },
                {
                    'issue': '過剰な権限',
