...
> **この章で学ぶこと**
> - OAuth 2.0 が生まれた背景（パスワードアンチパターンや OAuth 1.0 の課題）と設計思想を理解する  
> - 代表的なグラントタイプ（特に Authorization Code + PKCE と Client Credentials）の役割と使い分けを説明できる  
> - 安全な実装のために押さえるべきポイント（state, PKCE, トークンの有効期限と保存場所など）の全体像を把握する  
>  
> ここから先に登場する詳細な Python コード例は、実装イメージを掴むための「発展的なサンプル」です。まずは設計思想とフローの理解に集中し、余力があればコードを読みながら「どのタイミングでトークンを発行・保存・検証しているか」という流れを確認していく読み方を推奨します。
    ...
    def why_oauth2(self):
        ...
        oauth1_problems = {
            'signature_complexity': {
                'issue': '署名の計算が複雑',
                'oauth1_example': '''
                # （参考）OAuth 1.0 の署名計算イメージ
                # 実務ではライブラリがこの処理を隠蔽してくれるため、
                # 「毎リクエストで複雑な署名計算が必要だった」というイメージだけ掴めば十分です。
                # OAuth 1.0の署名計算
...
> **読み方の目安**
>  
> この節では、Authorization Code Grant の設計上のポイントとフローを理解することが主目的です。続く `AuthorizationCodeGrant` クラス以下のコードは、実装イメージを掴むための発展的サンプルであり、すべての行を追う必要はありません。「どのタイミングで認可コードを取得し、トークンエンドポイントに送り、アクセストークンを受け取っているか」という流れだけを意識して読んでも構いません。
