...
「パスワードを複雑にすれば安全」という単純な理解では、現代の脅威に対抗できません。この章では、認証技術の本質を整理し、なぜ多要素認証が必要なのか、生体認証にはどのような課題があるのかを学びます。
...
例（傾向の要約）：
- パスワード起因の侵害は多い
- 利用者は多数のアカウントを保有し、使い回しが起きやすい
- パスワードマネージャーの利用は限定的な場合がある
```
...
1. **ブルートフォース攻撃の高速化**
   ```python
   # GPUを使った攻撃速度の例
   # 数値はハードウェア/実装/パラメータで大きく変動するため要確認
   hash_rates = {
...
**事例（要確認）**：
- 大規模組織では、強制MFAやパスワードレス化によりフィッシング耐性が向上したと報告されることがあります。
- ただし、適用範囲・前提（対象者、測定期間、測定方法）で効果は変動します。
...
※ FAR/FRR等の数値は、製品・環境・運用条件で変動します（要確認）。
```python
biometric_types = {
    '生理的生体認証': {
        '指紋': {
