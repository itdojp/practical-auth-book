...
> **この章で学ぶこと**
> - 認証・認可がビジネスリスクとどのように結びついているかを、具体的な事例を通じて理解する
> - 認証／認可／アイデンティティ／セッション／トークンといった基本用語の違いを、自分の言葉で説明できるようになる
> - 認証技術の歴史的な流れを踏まえ、なぜ現在のベストプラクティスがその形になっているのかの概要を把握する
...
ある中規模のECサイトで発生した事故を考えてみましょう。このサイトは「簡単な会員機能」として、メールアドレスとパスワードのみの認証を実装していました。

```text
被害の概要：
- 不正アクセスによる個人情報流出：数万〜十万件規模（架空の例）
- 直接的な損害賠償：数千万円〜数億円規模
- ブランド価値の毀損：定量化が難しい
- 復旧にかかった期間：数週間〜数ヶ月
- 失った顧客の信頼：1年以上を要する場合がある
```
...
**コスト削減効果**：
1. **セキュリティインシデントの予防**
   - データ漏洩は賠償・対応コストが大きくなり得ます（規模・法令・契約により変動）。
   - 適切な認証・認可により、典型的な侵入経路（例: パスワードリスト攻撃）のリスクを下げられます。

2. **運用コストの削減**
   - パスワードリセット/本人確認の問い合わせ対応は運用負荷になります。
   - セルフサービス化（例: メール/アプリによる再設定）で一次対応の負荷を下げられます。

3. **コンプライアンス対応**
   - 例: GDPRでは、違反内容や事業規模などの条件により、全世界年間売上高の4%または2,000万ユーロのいずれか高い方を上限とする行政制裁金が科され得ます。
   - 多くの法令・規制・契約において、適切な認証・認可は重要な要件と位置付けられています（具体的な要件は法域・業界・契約内容に依存します）。

**ビジネス価値の向上**：
1. **顧客満足度の向上**
   - SSOにより、ログインの手間を減らし、ユーザー体験を改善できます。
   - 生体認証等により、パスワード忘れによる離脱を減らせます。
...
```text
□ ログイン処理 → 認証（Authentication）：「誰であるか」を確かめるプロセス
□ 「このボタンを押せるか」の判定 → 認可（Authorization）：「何をしてよいか」を決めるプロセス
□ 「誰であるか」の情報 → アイデンティティ（Identity）：ユーザーを一意に識別する属性の集合
